...
- [`docs/adr/0015-fallback-reuses-resumable-session.md`](../adr/0015-fallback-reuses-resumable-session.md)
...
| Fallback session | An eligible fallback reuses the previous OpenCode worker session when it is still resumable, sending the next profile's model on that same session; the pinned issue, branch, worktree, base identity, and chain position are unchanged. A fresh session is started only when no resumable session exists. Model is a per-prompt parameter, not a property of the session. | Checkpoint records failed profile, next profile, category, and chain position. Reuse requires a session id that passes the opaque-id rule and matches directory, issue, branch, and base identity. |
