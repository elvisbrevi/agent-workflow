...
2. Respect the repository instructions, issue dependencies, blockers, and
   ...
   claim it, implement and test it, inspect `az repos pr list` for the source
   branch, create a pull request with `az repos pr create` only when none
   exists, complete the unique pull request with `az repos pr update`, verify
   with `az repos pr list` that exactly one pull request is completed and
   succeeded into the configured base branch, and only then close the work item
   with `az boards work-item update --state`.
