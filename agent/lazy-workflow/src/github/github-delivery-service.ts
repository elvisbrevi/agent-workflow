...
export class GitHubDeliveryService implements GitHubDeliveryAdapter {
  ...
  async createOrReusePullRequest(issue: number, branch: string, baseBranch: string, commit: string, workingDirectory: string, closesIssue = true, issueReference = `#${issue}`, summary?: string): Promise<GitHubPullRequest> {
    ...
    // out of the body, so it leads: a summary long enough to be capped can never displace it.
    const closing = summary?.trim().slice(0, MAX_SUMMARY_CHARACTERS);
    const body = closing ? [reference, "", closing].join("\n") : reference;
  ...
  async closeIssue(issue: number, pullRequest: number, mergeCommit: string, workingDirectory: string): Promise<void> {
    ...
    // The marker is the whole comment: a rerun recognises the delivery by it, and the delivery's
    // own words are already in the pull request the marker names (ADR-0037).
