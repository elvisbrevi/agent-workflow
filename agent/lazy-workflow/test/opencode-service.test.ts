...
import { getDefaultReporter } from "../src/output/operator-output.ts";
...
/**
 * El watchdog de inactividad (ADR-0039): una sesión que deja de emitir se mata y
 * el resultado lo dice, para que la cadena de fallback descienda igual que ante un
 * agotamiento. Antes se la reanudaba con un empujón hacia un marcador que ya no
 * existe.
 */
test("una sesión silenciosa se termina y el resultado pide descender", async () => {
  let killed: string | null = null;
  let exit: (code: number) => void = () => undefined;
  // Un stdout que emite una línea y se queda abierto, como el de un proceso vivo: si el stream
  // cerrara, cerrar el stream sería la señal de fin y no habría silencio que medir.
  const openStdout = new ReadableStream<Uint8Array>({
    start(controller) {
      controller.enqueue(new TextEncoder().encode(
        `${JSON.stringify({ type: "text", sessionID: "ses_idle", part: { type: "text", text: "voy" } })}\n`,
      ));
    },
  });
  const service = new OpenCodeService(
    () => ({
      stdout: openStdout,
      stderr: new ReadableStream<Uint8Array>({ start() { /* abierto y callado */ } }),
      exited: new Promise<number>((resolve) => { exit = resolve; }),
      kill: (signal: string) => { killed = signal; exit(143); },
    }),
    getDefaultReporter(),
    5,
    20,
  ...
  const execution = await service.run({
    model: "m", variant: "high", session: null, prompt: "p", workingDirectory: "/repo",
  });

  expect(execution.idleTimedOut).toBeTrue();
  expect(execution.failed).toBeTrue();
  expect(execution.idleMs).toBe(20);
